...
class BktoolController extends Controller {
	...
	public function actionIndex() {
		$this->setPageTitle($this->getNumberOfRelevantUpdatesAsString() .
				' ' . $this->organisationName . '-BK');
		...
		$bkTool = new Bktool();
		$data = array();
		$data['members'] = $bkTool->getAllActiveMembersByGroupId(
				$this->bkGroupId, $_SESSION['orderby'], $_SESSION['order']);
	...
	public function actionEditcompanyform($id) {
		...
		if (isset($errordata['error'])) {
			...
			$bkForms->updateCompanyInformation($id, $_POST['editedcompany'], $_POST['mail'], $_POST['phonenumber'], $_POST['address'], $_POST['postbox'], $_POST['postnumber'], $_POST['postplace'], $_POST['homepage'], $parentCompanyId, $_POST['status'], $_POST['priority']);
	...
	public function actionAddcompanyform() {
		...
		if (isset($errordata['error'])) {
			...
			$bkForms->insertCompanyInformation($_POST['addedcompany'], $_POST['mail'], $_POST['phonenumber'], $_POST['address'], $_POST['postbox'], $_POST['postnumber'], $_POST['postplace'], $_POST['homepage'], $parentCompanyId, $_POST['status']);
			...
			if ($memberId != 0) {
				if (!$bkForms->isAlreadyGroupMember($memberId, $this->bkGroupId)) {
					...
					foreach ($data['member'] as $member) :
						$errordata[$i] = 'Brukeren ' . $member['firstName'] .
								' ' . $member['middleName'] . ' ' . $member['lastName'] .
								' er allerede et medlem og ble ikke lagt til';
					endforeach;
				}
			}
			$i += 1;
